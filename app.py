...
def _run_periodic_health_check():
    ...
    while True:
        ...
        try:
            ...
            logger.warning(f"Health check: Daily backup failed — {e}")
# ─────────────────────────────────────────────
# CSRF PROTECTION
# ─────────────────────────────────────────────
...
@app.route("/")
def pos():
    product_count = db.get_product_count(active_only=True)
    products = db.get_all_products(active_only=True, limit=100)
    categories = db.get_categories_from_products(products)
    settings = get_all_config()
    if product_count > 100:
        logger.info(f"Product grid truncated to 100 (total {product_count})")
    return render_template("pos.html", products=products, categories=categories, settings=settings, products_truncated=product_count > 100)
...
_barcode_cache = {}
_barcode_cache_timer = 0

def _get_cached_barcode(barcode):
    global _barcode_cache_timer
    now = time.time()
    if now - _barcode_cache_timer > 60:
        _barcode_cache.clear()
        _barcode_cache_timer = now
    ...
    if product:
        _barcode_cache[barcode] = product
    else:
        _barcode_cache[barcode] = _sentinel
    return product
...
def api_barcode_lookup():
    ...
    product = _get_cached_barcode(barcode)
    if not product:
