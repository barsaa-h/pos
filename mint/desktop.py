#!/usr/bin/env python3
"""
Linux Mint XFCE Desktop Launcher for POS System.

Double-click .desktop icon or run: python3 mint/desktop.py

Optimized for Linux Mint XFCE on a slow store computer (e.g. i5 2nd gen):
  - GTK backend via pywebview with WebKit2GTK
  - /dev/usb/lp* auto-detection for thermal printers
  - flock-based single-instance lock
  - systemd service support
  - XDG desktop integration
  - Reduced splash screen (lighter, smaller font)
  - Lazy-imported webview so the server can start before the UI loads
  - Lower-resolution placeholder until webview is ready (avoids white flash)
  - Honors XFCE optimizations (compositor off, no screensaver) — Mint-specific
"""
...
# WebKit2GTK optimizations for i5 2nd gen Intel HD Graphics (Ivy Bridge).
# Disabling compositing mode avoids GPU driver fallback to slow software
# rendering, making the UI dramatically smoother on this hardware.
...
# Mint typically uses XFCE display manager; default port matches Ubuntu launcher
FLASK_PORT = int(os.environ.get("POS_PORT", "8765"))
DEV_MODE = "--dev" in sys.argv
LOCK_FILE = os.path.join(tempfile.gettempdir(), "pos-app-mint.lock")

# File logging — same pattern as Ubuntu launcher
...
def run_flask(port):
    ...
    Mint optimization: use waitress when available (production-grade WSGI,
    lower CPU/memory than the dev server on slow hardware). Falls back to
    Flask's built-in server if waitress isn't installed.

    We set POS_SKIP_MODULE_INIT=1 so that `from app import create_app`
    doesn't also trigger the module-level _application = create_app() — which
    would spawn WAL checkpoint, GC, and heartbeat threads twice and double
    eBarimt retry cost. On slow i5 2nd gen this saves ~150ms of startup.
    """
...
def show_splash():
    """Mint splash is intentionally lighter than Ubuntu's.

    Smaller window, fewer widgets — important on i5 2nd gen where Tk render
    cost is noticeable during boot.
    """
    global _splash_root
    try:
        ...
        # Smaller splash for Mint (was 400x230 on Ubuntu)
        w, h = 320, 160
        x, y = (ws - w) // 2, (hs - h) // 2
        _splash_root.geometry(f"{w}x{h}+{x}+{y}")
        _splash_root.configure(bg="#1a1a2e")
        # Prefer system theme font; fall back to first available
        SPLASH_FONT = "Sans"
        for f in font.families(_splash_root):
            ...
            if fl.startswith("ubuntu") or fl == "dejavu sans" or fl == "liberation sans":
                SPLASH_FONT = f
                break
        # Single combined label instead of three — fewer Tk widgets to render
        tk.Label(
            _splash_root,
            text="Моност — POS\nАчааллаж байна...",
...
class POSApplication:
    ...
    def start(self):
        ...
        try:
            ...
            with get_db() as conn:
                ...
                # Enable low_perf_mode on the Mint launcher by default — store
                # computers are typically i5 2nd gen + Intel HD graphics where
                # CSS animations and backdrop-filter cost noticeable frames.
                # Owners can toggle this off in /settings if hardware improves.
                row = conn.execute(
    ...
    def _start_windows(self):
        # Lazy import — webview is heavy and we want the server up first
        import webview
        ...
        # Mint: skip the menu bar — store computers don't need it and it
        # costs frames to render. Keep the help shortcut via F1 in the page.
