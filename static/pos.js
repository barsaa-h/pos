...
(function() {
    var saved = sessionStorage.getItem('pos_cart');
    if (saved) {
    ...
    if (products.length === 0) {
        ...
        products.forEach(function(p) {
            html += '<div class="product-card' + (!p.barcode && p.unit !== 'кг' ? ' no-barcode' : '') + '"'
                 ...
                 + (p.image_url ? '<img src="' + p.image_url + '" alt="' + p.name.replace(/'/g, '\\x27') + '" class="card-icon" style="width:3.2rem; height:3.2rem; object-fit:cover; border-radius:8px; display:block;">'
...
function filterProducts() {
    ...
    requestAnimationFrame(function() {
        for (let i = 0; i < len; i++) {
            const card = cards[i];
            const matchSearch = !query || card.dataset.name.indexOf(query) !== -1 || card.dataset.barcode.indexOf(query) !== -1;
